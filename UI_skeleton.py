...
with st.sidebar:
    ...
    st.metric("External APIs", "GLEIF / OpenSanctions")
    st.markdown("---")
...
with tab1:
    ...
    with col_in:
        ...
        with st.form("vendor_onboarding_form"):
            ...
            v_country = st.selectbox("Operating Country", ["India", "United States", "Denmark", "United Kingdom", "Germany", "USA", "UK"])
    ...
    with col_out:
        ...
        if submit_btn and v_name:
            ...
            justification = outcome.get("business_justification", "No justification provided.")
            ...
            sanctions_display = "Flagged" if sanctions_hit else ("Clear" if sanctions_hit is False else "Unverified")
            s_color = "inverse" if sanctions_hit else "normal"
            
            hierarchy = outcome.get("hierarchy_relationship_found")
            hierarchy_display = "Found" if hierarchy else ("None" if hierarchy is False else "N/A")

            m1.metric("Composite Score", f"{comp_score}%", delta=f"Fuzzy: {fuzzy}%", delta_color="off")
            m2.metric("Vector Semantic", f"{semantic}%", delta_color="off")
            m3.metric("UN Sanctions", sanctions_display, delta="Risk Check", delta_color=s_color)
            m4.metric("GLEIF Hierarchy", hierarchy_display, delta="Parent/Sub Check", delta_color="off")

            if outcome.get("related_parent_entity"):
                st.caption(f"**Discovered Parent Entity:** {outcome.get('related_parent_entity')}")

            # Dynamic Actions
            st.markdown("---")
            act_col1, act_col2 = st.columns(2)
            if decision == "AUTO_APPROVE":
                act_col1.button("➕ Proceed to Onboard New Vendor", type="primary", use_container_width=True)
            elif decision == "MANUAL_REVIEW":
                if match_result:
                    act_col1.button(f"✅ Consolidate to {match_result.get('vendor_id')}", type="primary", use_container_width=True)
                act_col2.button("📩 Route to Data Steward", use_container_width=True)
            elif decision == "AUTO_REJECT":
                act_col1.button("🔒 Acknowledge & Block Request", type="primary", use_container_width=True)
